...
def failed_sent(line, receiver, channel_name):
    command = line.split()
    filepath = command[1]
    sender = command[2]
...
def success_sent(line, receiver, channel_name):
    command = line.split()
    filepath = command[1]
    sender = command[2]
    sender_socket = client_info[channel_name][sender][0]
                ...
                with lock:
                    while "\n" in data:
                        ...
                        if message[:6] == "$User:":
                            ...
                            failed_sent(message, username, channel_name)
                        elif message[:13] == "$Success-sent":
                            success_sent(message, username, channel_name)
