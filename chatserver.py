...
channel_port = []
channel_capacity = []
channel_users = {}  # {channel_names: [user_1, user_2]}
client_address_users = {}
...
def handle_client_messages(message, index, client_address):
    global channel_users
    global client_address_users
    if message[:7] == "$User: ":
        client_username = message[7:]
        channel_name = channel_names[index]
        client_address_users[client_address] = [client_username]
        if duplicate_usernames(client_username, channel_name):
            return f"$UserError: {channel_name}"
        channel_users[channel_name].append(client_username)
    ...
    with client_socket:
        try:
            while data := client_socket.recv(BUFSIZE):
                message = handle_client_messages(data.decode(), index, client_address)
                client_socket.sendall(message.encode())
        except Exception:
            pass  # close the client's connection if they abruptly close
    # error or EOF - client disconnected
    client_socket.close()
    if not (len(message) > 10 and message[:10] == "$UserError"):
        print(f"Connection from {client_address_users[client_address][0]} closed.")
